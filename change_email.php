...
<?php
    ...
    if (isset($_POST['save'])) {
        ...
        $sql = 'SELECT COUNT(accUsername) as num FROM accounts WHERE (accUsername = ?) AND (accId != ?)';
        $stmt = $conn->prepare($sql);
        $stmt->bind_param('si', $email, $account['accId']);
        $stmt->execute();
        $numAcc = $stmt->get_result()->fetch_assoc()['num'];

        $isValid = true;
        if (empty($email)) {
            $msgEmail = 'This is a required field';
            $isValid = false;
        }
        if (!filter_var($email, FILTER_VALIDATE_EMAIL)) {
            $msgEmail = 'Please enter a valid email';
            $isValid = false;
        }
        if ($numAcc > 0) {
        ...
        if (!password_verify($password, $account['accPassword'])) {
            $msgCurrent = 'Wrong password';
            $isValid = false;
        }
        if ($isValid) {
            $sqlEditInfo = 'UPDATE accounts SET accUsername = ? WHERE accUsername = ?';
            $stmt = $conn->prepare($sqlEditInfo);
            $stmt->bind_param('ss', $email, $_SESSION['customer']);
            $stmt->execute();

            $sqlUpdateAcc = 'UPDATE customers SET cusEmail = ? WHERE cusEmail = ?';
            $stmt = $conn->prepare($sqlUpdateAcc);
            $stmt->bind_param('ss', $email, $_SESSION['customer']);
            $stmt->execute();
                    ...
                    <div class="myaccount-wrapper">
                        ...
                        <div class="myaccount-content">
                            <div>
                                ...
                                <h1 class="my-account__page-title">Change Email</h1>
                                <form class="account-information " method="post">
                                    <div class="change-password-form ">
                                        <div class="field-wrapper  can-focus  "><label for="email" class="label">Email<span class="requiredSymbol">*</span></label>
                                            <div class="field-icons" style="--iconsBefore: 0; --iconsAfter: 0;"><span class="input-block"><input class="input-box-new-design" field="email" type="text" name="email" value="<?= isset($email) ? $email : $_SESSION['customer']  ?>"></span><span class="before"></span><span class="after"></span></div><span class="message-root">
                                                <p class="root_error"><?= !isset($msgEmail) ? '' : $msgEmail  ?></p>
                                            </span>
                                        </div>
                                        <div class="field-wrapper  can-focus  "><label for="password" class="label">Current Password<span class="requiredSymbol">*</span></label>
                                            <div class="field-icons" style="--iconsBefore: 0; --iconsAfter: 0;"><span class="input-block"><input class="input-box-new-design" field="password" type="password" name="password" value=""></span><span class="before"></span><span class="after"></span></div><span class="message-root">
